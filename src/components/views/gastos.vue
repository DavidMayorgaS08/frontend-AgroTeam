    ...
    <div class="cont_form" v-if="formulario">
      <div class="form">
        ...
        <div class="titulo_form">
          <p v-if="variable === 0" class="text_titulo_form">crear gasto</p>
          <p v-else class="text_titulo_form">editar gasto</p>
        </div>
        ...
        <div class="cont_inputs">
          <p class="text_inputs">NroFactura</p>
          <input type="text" class="inputs" v-model="NroFactura">
        </div>
        <div class="cont_inputs">
        ...
        <div class="cont_inputs">
          <p class="text_inputs">semillas</p>
          <select required v-model="semillaOption">
            <option value="" disabled selected hidden></option>
            <option
            v-for="(semilla, index) in semillas"
            :key="semilla._id"
            :value="index + 1"
            >{{ semilla.especieVariedad }}</option>
          </select>
        </div>
        <div class="cont_inputs">
          <p class="text_inputs">mantenimiento</p>
          <select required v-model="mantenimientoOption">
            <option value="" disabled selected hidden></option>
            <option
            v-for="(mantenimiento, index) in mantenimientos"
            :key="mantenimiento._id"
            :value="index + 1"
            >{{ mantenimiento.fecha.split("T")[0] }}</option>
          </select>
        </div>
...
let listarInactivos = async () => {
  spinner.value = true;
  r = await useGastos.getInactivos();
  rows.value = r.gasto;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useGastos.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useGastos.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let semillaOption = ref("");
let mantenimientoOption = ref("");
let estado = ref(1);
...
let validaciones = () => {
  ...
  if (NroFactura.value === "" || String(NroFactura.value).trim() === "") {
...
let crear = async () => {
  ...
  await useSemillas.getSemillas();
  await useMantenimientos.getMantenimientos();
  insumos.value = useInsumos.insumos.insumos;
  semillas.value = useSemillas.semillas.semillas;
  mantenimientos.value = useMantenimientos.mantenimientos.mantenimiento;
  variable.value = 0;
...
let editar = async (data) => {
  ...
  NroFactura.value = data.NroFactura;
  descripcion.value = data.descripcion;
  total.value = data.total;
  insumoOption.value = insumos.value.findIndex((i) => i._id === data.id_insumo) + 1;
  semillaOption.value = semillas.value.findIndex((s) => s._id === data.id_semillas) + 1;
  mantenimientoOption.value = mantenimientos.value.findIndex((m) => m._id === data.id_mantenimiento) + 1;
  formulario.value = true;
  spinner.value = false;
}
...
let enviarCrear = async () => {
  ...
  let data = {
    ...
    id_semillas: semillas.value[semillaOption.value - 1]._id,
    id_mantenimiento: mantenimientos.value[mantenimientoOption.value - 1]._id,
    estado: estado.value,
  };
  spinner.value = true;
...
let enviarEditar = async () => {
  ...
  spinner.value = true;
  await useGastos.putGasto(id.value, data);
  cerrarForm();
  spinner.value = false;
