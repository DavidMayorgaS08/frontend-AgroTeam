    ...
    <div class="cont_form" v-if="formulario">
      <div class="form">
        ...
        <div class="titulo_form">
          <p v-if="variable === 0" class="text_titulo_form">crear nómina</p>
          <p v-else class="text_titulo_form">editar nómina</p>
        </div>
...
import { useNominasStore} from "../../stores/nominas.js";
import { useEmpleadosStore } from "../../stores/empleados.js";

let useNominas = useNominasStore();
...
let columns = ref([
  ...
  },
  {
    ...
    field: (row) => {
      ...
      empleado = empleado.find((empleado) => empleado._id == row.id_empleado);
...
let listarInactivos = async () => {
  spinner.value = true;
  r = await useNominas.getInactivos();
  rows.value = r.nomina;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useNominas.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useNominas.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let editar = async (data) => {
  ...
  empleadoOption.value = empleados.value.findIndex((empleado) => empleado._id == data.id_empleado) + 1;
...
let enviarCrear = async () => {
  ...
  spinner.value = true;
  await useNominas.postNominas(data);
  cerrarForm();
  spinner.value = false;
...
let enviarEditar = async () => {
  ...
  spinner.value = true;
  await useNominas.putNominas(id.value, data);
  cerrarForm();
  spinner.value = false;
