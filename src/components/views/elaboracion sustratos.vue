 <template>
  <div class="app">
    ...
    <div class="cont_nombre_vue">
      <p class="nombre_vue">elaboracion sustratos</p>
    </div>
    <div class="q-pa-md">
    ...
    <div class="cont_form" v-if="formulario">
      <div class="form">
        ...
        <div class="titulo_form">
          <p v-if="variable === 0" class="text_titulo_form">crear elaboración de sustrato</p>
          <p v-else class="text_titulo_form">editar elaboración de sustrato</p>
        </div>
        <div class="part1">
          ...
          <div class="cont_inputs">
            <p class="text_inputs">producto comercial</p>
            <input type="text" class="inputs" v-model="productoComercial">
          </div>
          <div class="cont_inputs">
            <p class="text_inputs">ingrediente activo</p>
            <input type="text" class="inputs" v-model="ingredenteActivo">
          </div>
          <div class="cont_inputs">
            <p class="text_inputs">dosis utilizada</p>
            <input type="text" class="inputs" v-model="dosisUtilizada">
          </div>
        </div>
        <div class="part2">
          <div class="cont_inputs">
            <p class="text_inputs">metodo aplicacion</p>
            <input type="text" class="inputs" v-model="metodoAplicacion">
          </div>
          <div class="cont_inputs">
            <p class="text_inputs">empleado operario</p>
          ...
          <div class="cont_inputs">
            <p class="text_inputs">empleado responsable</p>
            <select required v-model="empleadoResponsableOption">
              <option 
              v-for="(empleado, index) in empleados"
              :key="empleado._id"
              :value="index + 1"
...
let columns = ref([
  ...
  {
    name: "id_empleado_operario",
    label: "Empleado Operario",
    align: "center",
    field: (row) => {
      let empleado = e.value.empleado;
      empleado = empleado.find((e) => e._id == row.id_empleado_operario);
  ...
  },
  {
    ...
    field: (row) => {
      let empleado = e.value.empleado;
      empleado = empleado.find((e) => e._id == row.id_empleado_responsable);
      return empleado.nombre;
    }
  },
  {
...
let listarActivos = async () => {
  spinner.value = true;
  r = await useElaboracionSustratos.getActivos();
  rows.value = r.elaboracionSustratos;
  spinner.value = false;
};
...
let listarInactivos = async () => {
  spinner.value = true;
  r = await useElaboracionSustratos.getInactivos();
  rows.value = r.elaboracionSustratos;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useElaboracionSustratos.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useElaboracionSustratos.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let empleadoResponsableOption = ref("");
...
let validaciones = () => {
 ...
 text.value = "La fecha es obligatoria";
  ...
  if(productoComercial.value === "" || productoComercial.value.trim() === "") {
 text.value = "El producto comercial es obligatorio";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(ingredenteActivo.value === "" || ingredenteActivo.value.trim() === "") {
 text.value = "El ingrediente activo es obligatorio";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(dosisUtilizada.value === "" || String(dosisUtilizada.value).trim() === "") {
 text.value = "La dosis utilizada es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(metodoAplicacion.value === "" || metodoAplicacion.value.trim() === "") {
 text.value = "El método de amplicación es obligatorio";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(empleadooperadorOption.value === "") {
 text.value = "El empleado operador es obligatorio";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(empleadoResponsableOption.value === "") {
 text.value = "El empleado responsable es obligatorio";
    registroFallido.value = true;
    ocultar();
    return false;
  }
...
let vaciarCampos = () => {
  ...
  empleadoResponsableOption.value = "";
  observaciones.value = "";
}
...
let editar = async (data) => {
  ...
  dosisUtilizada.value = data.dosisUtilizada;
  metodoAplicacion.value = data.metodoAplicacion;
  empleadooperadorOption.value = empleados.value.findIndex((e) => e._id === data.id_empleado_operario) + 1;
  empleadoResponsableOption.value = empleados.value.findIndex((e) => e._id === data.id_empleado_responsable) + 1;
...
let enviarCrear = async () => {
  ...
  let data = {
    ...
    id_empleado_operario: empleados.value[empleadooperadorOption.value - 1]._id,
    id_empleado_responsable: empleados.value[empleadoResponsableOption.value - 1]._id,
    observaciones: observaciones.value,
    estado: estado.value
  };
  spinner.value = true;
  await useElaboracionSustratos.postElaboracionSustratos(data);
  cerrarForm();
  spinner.value = false;
...
let enviarEditar = async () => {
  ...
  spinner.value = true;
  await useElaboracionSustratos.putElaboracionSustratos(id.value, data);
  cerrarForm();
  spinner.value = false;
