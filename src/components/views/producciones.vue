<template>
  <div class="app">
    ...
    <div class="cont_nombre_vue">
      <p class="nombre_vue">Producciones</p>
    </div>
    <div class="q-pa-md">
    ...
    <div class="cont_form" v-if="formulario">
      <div class="form">
        ...
        <div class="titulo_form">
          <p v-if="variable === 0" class="text_titulo_form">crear producción</p>
          <p v-else class="text_titulo_form">editar producción</p>
        </div>
        <div class="part1">
          ...
          <div class="cont_inputs">
            <p class="text_inputs">Nro Lote</p>
        ...
        <div class="part2">
          ...
          <div class="cont_inputs">
            <p class="text_inputs">cantidad trabajadores</p>
            <input type="number" class="inputs" v-model="cantidadTrabajadores">
          </div>
          <div class="cont_inputs">
...
let listarTodos = async () => {
  spinner.value = true;
  r = await useProducciones.getProducciones();
  c.value = await useCultivos.getCultivos();
  rows.value = r.produccion;
  spinner.value = false;
};

let listarActivos = async () => {
  spinner.value = true;
  r = await useProducciones.getActivos();
  rows.value = r.produccion;
  spinner.value = false;
};
...
let listarInactivos = async () => {
  spinner.value = true;
  r = await useProducciones.getInactivos();
  rows.value = r.produccion;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useProducciones.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useProducciones.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let validaciones = () => {
  ...
  if(cantidad.value === "" || String(cantidad.value).trim() === "") {
   text.value = "La cantidad = true";
    ocultar();
    return false;
  }
  if(cantidadTrabajadores.value === "" || String(cantidadTrabajadores.value).trim() === "") {
   text.value = "La cantidad de trabajadores es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
...
let editar = async (data) => {
  ...
  cantidadTrabajadores.value = data.cantidadTrabajadores;
...
let enviarCrear = async () => {
  ...
  spinner.value = true;
  await useProducciones.postProducciones(data);
  cerrarForm();
  spinner.value = false;
...
let enviarEditar = async () => {
  ...
  spinner.value = true;
  await useProducciones.putProducciones(id.value, data);
  cerrarForm();
  spinner.value = false;
