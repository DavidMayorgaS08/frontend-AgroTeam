    ...
    <div class="cont_form" v-if="formulario">
      <div class="form">
        ...
        <div class="titulo_form">
          <p v-if="variable === 0" class="text_titulo_form">crear cultivo</p>
          <p v-else class="text_titulo_form">editar cultivo</p>
        </div>
...
let listarTodos = async () => {
  ...
  p.value = await useParcela.getParcelas();
  rows.value = r.cultivo;
  spinner.value = false;
};

let listarActivos = async () => {
  spinner.value = true;
  r = await useCultivo.getActivos();
  rows.value = r.cultivo;
  spinner.value = false;
};
...
let listarInactivos = async () => {
  spinner.value = true;
  r = await useCultivo.getInactivos();
  rows.value = r.cultivo;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useCultivo.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useCultivo.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let validaciones = () => {
  ...
  if (parcelaOption.value === "") {
   text.value = "La parcela es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
...
let crear = async () => {
  spinner.value = true;
  await useParcela.getParcelas();
  parcelas.value = useParcela.parcelas.parcela;
  variable.value = 0;
...
let editar = async (data) => {
  ...
  parcelaOption.value = parcelas.value.findIndex(
    (p) => p._id == data.id_parcela
  ) + 1;
  formulario.value = true;
  spinner.value = false;
  console.log(parcelaOption.value);
