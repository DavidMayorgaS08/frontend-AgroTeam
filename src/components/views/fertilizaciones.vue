<template>
  <div class="app">
    ...
    <div class="cont_nombre_vue">
      <p class="nombre_vue">fertilizaciones</p>
    </div>
    <div class="q-pa-md">
    ...
    <div class="cont_form" v-if="formulario">
      <div class="form">
        ...
        <div class="titulo_form">
          <p v-if="variable === 0" class="text_titulo_form">crear fertilización</p>
          <p v-else class="text_titulo_form">editar fertilización</p>
        </div>
        <div class="part1">
...
let listarActivos = async () => {
  spinner.value = true;
  r = await useFertilizaciones.getActivos();
  rows.value = r.fertilizaciones;
  spinner.value = false;
};

let listarInactivos = async () => {
  spinner.value = true;
  r = await useFertilizaciones.getInactivos();
  rows.value = r.fertilizaciones;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useFertilizaciones.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useFertilizaciones.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
