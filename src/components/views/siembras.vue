    ...
    <div class="cont_form" v-if="formulario">
      <div class="form">
        ...
        <div class="titulo_form">
          <p v-if="variable === 0" class="text_titulo_form">crear siembra</p>
          <p v-else class="text_titulo_form">editar siembra</p>
        </div>
        <div class="part1">
...
let columns = ref([
  ...
  },
  {
    ...
    field: (row) => {
      ...
      Semilla = Semilla.find((s) => s._id == row.id_semilla);
...
let listarActivos = async () => {
  spinner.value = true;
  r = await useSiembras.getActivos();
  rows.value = r.siembras;
  spinner.value = false;
};

let listarInactivos = async () => {
  spinner.value = true;
  r = await useSiembras.getInactivos();
  rows.value = r.siembras;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useSiembras.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useSiembras.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let validaciones = () => {
  ...
  if(fechaSiembra.value === "") {
   text.value = "La fecha de la siembra es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(fechaCosecha.value === "") {
   text.value = "La fecha de la cosecha es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  ...
  if(semillaOption.value === "") {
   text.value = "La semilla es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
...
let editar = async (data) => {
  ...
  semillaOption.value = semillas.value.findIndex((s) => s._id === data.id_semilla) + 1;
  formulario.value = true;
  spinner.value = false;
}
...
let enviarCrear = async () => {
  ...
  let data = {
    ...
    id_semilla: semillas.value[semillaOption.value - 1]._id,
    estado: estado.value
  };
  spinner.value = true;
