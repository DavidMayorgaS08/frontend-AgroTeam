<template>
  <div class="app">
    ...
    <div class="cont_nombre_vue">
      <p class="nombre_vue">Compradores</p>
    </div>
    <div class="q-pa-md">
...
let useProduccion = useProduccionesStore();
...
let columns = ref([
  {
    ...
    align: "center",
    field: (row) => {
      let produccion = p.value.produccion
      produccion = produccion.find((p) => p._id == row.id_produccion);
      return produccion.NroLote + " - " + produccion.especie;
    },
  },
  {
    name: "fecha",
  ...
  {
    name: "NroGuiaTransporte",
    label: "Nro Guía de transporte",
    align: "center",
...
let listarTodos = async () => {
  spinner.value = true;
  r = await useComprador.getCompradores();
  p.value = await useProduccion.getProducciones();
  rows.value = r.compradores;
  spinner.value = false;
};

let listarActivos = async () => {
  spinner.value = true;
  r = await useComprador.getActivos();
  rows.value = r.compradores;
  spinner.value = false;
};
...
let listarInactivos = async () => {
  spinner.value = true;
  r = await useComprador.getInactivos();
  rows.value = r.compradores;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useComprador.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useComprador.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let validaciones = () => {
  ...
  if(especie.value === "" || especie.value.trim() === "") {
  ...
  if(NroGuiaTransporte.value === "" || NroGuiaTransporte.value.trim() === "") {
    alert("El Nro de Guía de transporte es obligatorio");
    return false;
  }
  if(NroLoteComecial.value === "" || NroLoteComecial.value.trim() === "") {
    alert("El Nro de Lote Comercial es obligatorio");
    return false;
  }
}

let vaciarCampos = () => {
  produccionOption.value = "";
  fecha.value = "";
  especie.value = "";
  nombre.value = "";
  telefono.value = "";
...
let crear = async () => {
  spinner.value = true;
  await useProduccion.getProducciones();
  produccion.value = useProduccion.producciones.produccion;
...
let editar = async (data) => {
  ...
  variable.value = 1;
  id.value = data._id;
  produccionOption.value = produccion.value.findIndex((p) => p._id == data.id_produccion) + 1;
...
let enviarCrear = async () => {
  ...
  let data = {
    id_produccion: produccion.value[produccionOption.value - 1]._id,
    fecha: fecha.value,
  ...
  spinner.value = true;
  await useComprador.postCompradores(data);
  vaciarCampos();
  spinner.value = false;
}
...
let enviarEditar = async () => {
  ...
  spinner.value = true;
  await useComprador.putCompradores(id.value, data);
  vaciarCampos();
  spinner.value = false;
}
