<template>
  <div class="app">
    ...
    <div class="cont_nombre_vue">
      <p class="nombre_vue">Proveedores</p>
    </div>
    <div class="q-pa-md">
...
let listarActivos = async () => {
  spinner.value = true;
  r = await useProveedores.getActivos();
...
let listarInactivos = async () => {
  spinner.value = true;
  r = await useProveedores.getInactivos();
  rows.value = r.proveedores;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useProveedores.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useProveedores.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
