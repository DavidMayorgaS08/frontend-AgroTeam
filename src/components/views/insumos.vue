...
let listarInactivos = async () => {
  spinner.value = true;
  r = await useInsumos.getInactivos();
  rows.value = r.insumos;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useInsumos.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useInsumos.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let validaciones = () => {
  ...
  if (relacionNPK.value === "" || relacionNPK.value.trim() === "") {
    text.value = "La relación NPK es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  ...
  if (unidad.value === "" || String(unidad.value).trim() === "") {
    text.value = "La unidad es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
...
let enviarCrear = async () => {
  ...
  spinner.value = true;
  await useInsumos.postInsumos(data);
  cerrarForm();
  spinner.value = false;
...
let enviarEditar = async () => {
  ...
  spinner.value = true;
  await useInsumos.putInsumo(id.value, data);
  cerrarForm();
  spinner.value = false;
