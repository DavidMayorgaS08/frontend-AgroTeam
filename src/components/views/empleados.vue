<template>
  <div class="app">
    ...
    <div class="cont_nombre_vue">
      <p class="nombre_vue">empleados</p>
    </div>
    <div class="q-pa-md">
...
let listarActivos = async () => {
  spinner.value = true;
  r = await useEmpleados.getActivos();
  rows.value = r.empleado;
  spinner.value = false;
};
...
let listarInactivos = async () => {
  spinner.value = true;
  r = await useEmpleados.getInactivos();
  rows.value = r.empleado;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useEmpleados.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useEmpleados.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
