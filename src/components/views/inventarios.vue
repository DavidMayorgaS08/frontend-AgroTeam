<template>
  <div class="app">
    ...
    <div class="cont_nombre_vue">
      <p class="nombre_vue">inventarios</p>
    </div>
    <div class="q-pa-md">
...
let listarTodos = async () => {
  spinner.value = true;
  r = await useInventario.getInventarios();
  rows.value = r.inventarios;
  spinner.value = false;
};

let listarActivos = async () => {
  spinner.value = true;
  r = await useInventario.getActivos();
  rows.value = r.inventarios;
  spinner.value = false;
};
...
let listarInactivos = async () => {
  spinner.value = true;
  r = await useInventario.getInactivos();
  rows.value = r.inventarios;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useInventario.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useInventario.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
