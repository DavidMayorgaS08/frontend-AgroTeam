    ...
    <div class="cont_form" v-if="formulario">
      <div class="form">
        ...
        <div class="titulo_form">
          <p v-if="variable === 0" class="text_titulo_form">crear riego</p>
          <p v-else class="text_titulo_form">editar riego</p>
        </div>
...
let validaciones = () => {
  ...
  if(diasTransplante.value === "" || String(diasTransplante.value).trim() === ""){
   text.value = "Los días del trasplante son obligatorios";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  ...
  if(horaInicio.value === ""){
   text.value = "La hora de inicio es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(horaFin.value === ""){
   text.value = "La hora de fin es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(dosis.value === "" || String(dosis.value).trim() === ""){
   text.value = "La dosis es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(cantidadAgua.value === "" || String(cantidadAgua.value).trim() === ""){
   text.value = "La cantidad de agua es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
...
let editar = async (data) => {
  ...
  diasTransplante.value = data.diasTransplante;
  estadoFenologico.value = data.estadoFenologico;
  ...
  cantidadAgua.value = data.cantidadAgua;
  formulario.value = true;
  spinner.value = false;
}
...
let enviarCrear = async () => {
  ...
  let horaInicioF = "2024-01-01T" + horaInicio.value + ":00.000Z";
  let horaFinF = "2024-01-01T" + horaFin.value + ":00.000Z";
  let data = {
  ...
  spinner.value = true;
  await useRiegos.postRiegos(data);
  cerrarForm();
  spinner.value = false;
...
let enviarEditar = async () => {
  ...
  spinner.value = true;
  await useRiegos.putRiegos(id.value, data);
  cerrarForm();
  spinner.value = false;
