    ...
    <div class="cont_form" v-if="formulario">
      <div class="form">
        ...
        <div class="titulo_form">
          <p v-if="variable === 0" class="text_titulo_form">crear proceso</p>
          <p v-else class="text_titulo_form">editar proceso</p>
        </div>
        ...
        <div class="cont_inputs">
          <p class="text_inputs">fecha inicio</p>
          <input type="date" class="inputs" v-model="fechaInicio">
        </div>
        <div class="cont_inputs">
          <p class="text_inputs">fecha fin</p>
          <input type="date" class="inputs" v-model="fechaFin">
        </div>
...
let listarTodos = async () => {
  spinner.value = true;
  r = await useProcesos.getProcesos();
...
let listarActivos = async () => {
  spinner.value = true;
  r = await useProcesos.getActivos();
  rows.value = r.proceso;
  spinner.value = false;
};
...
let listarInactivos = async () => {
  spinner.value = true;
  r = await useProcesos.getInactivos();
  rows.value = r.proceso;
  spinner.value = false;
};
...
let activar = async (row) => {
  spinner.value = true;
  r = await useProcesos.putActivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let desactivar = async (row) => {
  spinner.value = true;
  r = await useProcesos.putDesactivar(row._id);
  listarTodos();
  spinner.value = false;
};
...
let validaciones = () => {
  ...
  if(fechaInicio.value ===""){
   text.value = "La fecha de inicio es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(fechaFin.value ===""){
   text.value = "La fecha de fin es obligatoria";
    registroFallido.value = true;
    ocultar();
    return false;
  }
...
let editar = async (data) => {
  ...
  fechaInicio.value = data.fechaInicio.split("T")[0];
  fechaFin.value = data.fechaFin.split("T")[0];
  formulario.value = true;
  spinner.value = false;
}
...
let enviarCrear = async () => {
  ...
  spinner.value = true;
  await useProcesos.postProcesos(data);
  cerrarForm();
  spinner.value = false;
...
let enviarEditar = async () => {
  ...
  spinner.value = true;
  await useProcesos.putProcesos(id.value, data);
  cerrarForm();
  spinner.value = false;
